      ...
      <div class="p-5 mb-4 rounded-lg bg-glass hover:border hover:border-gray-500">
        <header class="flex items-center justify-between mb-3 text-white">
          <div class="flex items-center">
            ...
            <span class="text-sm font-light">{{\Carbon\Carbon::parse($meet->date_time)->translatedFormat('l, d F Y')}}</span>
            <span class="text-sm font-light ms-2"> - {{\Carbon\Carbon::parse($meet->date_time)->format('H:i') }} WIB</span>
          </div>
            ...
            @role(['admin'])
              ...
              <div class="inline md:flex md:justify-between">
                ...
                <div class="flex gap-2 text-gray-400 md:gap-4">
                  <button type="button" wire:click="delete('{{$meet->slug}}')" class="flex gap-1 rounded-md items-center text-base font-medium border hover:text-red-600 border-[#27272A] px-2 md:px-4 py-1">
