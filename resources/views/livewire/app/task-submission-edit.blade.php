<div>
  ...
  <header class="flex my-7">
    <h2 class="text-2xl font-bold text-white md:text-3xl">Edit Jawaban</h2>
  </header>
  {{-- Header End --}}
  ...
  <form class="mb-10" wire:submit.prevent="update">
    @csrf
      ...
      <div class="grid grid-cols-1 gap-4 mb-4">
        <div>
          ...
          <input type="link" id="task_submission" wire:model="submission"
            class="block w-full p-3 text-white rounded-lg bg-lightGray">
          @error('submission')
    ...
    <section class="flex items-center justify-end">
      ...
      <button class="flex items-center px-5 py-3 text-sm font-semibold text-black bg-white rounded-md"
        wire:loading.remove>
        Update Jawaban <iconify-icon icon="fa-solid:save" class="text-xl ms-2"></iconify-icon>
      </button>
